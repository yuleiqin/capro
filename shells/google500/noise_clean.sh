...
export resume_path=${SAVE_DIR}/bopro/checkpoint_best_imgnet.tar
...
--root_dir_test_web ${root_dir_test_web} --pathlist_test_web ${pathlist_test_web} \
--root_dir_test_target ${root_dir_test_target} --pathlist_test_target ${pathlist_test_target} \
--arch "resnet50" --workers 8 --epochs 20 \
--frozen_encoder_epoch 21 --init-proto-epoch 21 --start_clean_epoch 21 \
--batch-size ${N_BATCHSIZE} --lr 1e-4 --cos --warmup_epoch 0 --beta 0.1 \
--exp-dir ${SAVE_DIR1} --start-epoch 0 --resume ${resume_path} \
--num-class ${N_CLASSES} --w-inst 0 --w-recn 0 --w-proto 0 \
