    ...
    def _parser(feature_list):
        ...
        for key, feature in feature_list: 
            if key == 'image':
                ...
                # if not image_raw.endswith(b'\xff\xd9'):
                #     image_raw = image_raw + b'\xff\xd9'
...
class webvision_dataset(Dataset):
    ...
    def skip_low_resolution_images(self):
        samples_hr = []
        targets_hr = []
        domains_hr = []
        meta_weights_hr = []
        ...
        for idx, sample in enumerate(self.samples):
            tfrecord_path, offset = sample
            if not "_google_hi" in tfrecord_path:
                continue
            samples_hr.append(self.samples[idx])
            targets_hr.append(self.targets[idx])
            domains_hr.append(self.domains[idx])
            meta_weights_hr.append(self.meta_weights[idx])

        self.samples, self.targets, self.domains, self.meta_weights = samples_hr, targets_hr, domains_hr, meta_weights_hr

        print("[TRAIN] Only Keep HighResolution Google Images")
        print("number of samples", len(self.samples),\
            "number of labels", len(self.targets),\
                "number of domain labels", len(self.domains),\
                    "number of target domains", np.sum(np.array(self.domains)),\
    ...
    def check_clean_sample_by_rank_distance(self):
        ...
        for sample, target, meta_weight in zip(self.samples, self.targets, self.meta_weights):
            ...
            imgs_by_target[target].append([sample, float(meta_weight_dists[0])])

        samples_fewshot_pseudo = set()
        ...
        for target in tqdm(target_list):
            ...
            num_imgs_sorted = max(1, min(len(img_list), abs(self.topk)))
            ### 直接根据原始网图类别的meta weight (rerank)进行排序
            num_fewshots_by_target.append([num_imgs_sorted, target])
            ...
            for img_item in img_list_sampled:
                ...
                samples_fewshot_pseudo.add(sample_name)
        ...
        print("=========><=========")
        print("number of pseudo few-shot samples", len(samples_fewshot_pseudo))
    ...
    def load_by_json(self):
        ...
        targets = []
        for target in annotation_json['targets']:
            targets.append(target[0])
        self.targets = targets
        self.domains = annotation_json['domains']
        self.meta_weights = [[[target], [1.0], [0.0]] for target in self.targets]
        print("=========><=========")
        print("[AnnotationJson] #{} samples; #{} labels; #{} domains (#{} target domains); #{} meta weights".format(
            len(self.samples), len(self.targets), len(self.domains), np.sum(np.array(self.domains)), len(self.meta_weights)))
        print("=========><=========")
        return
        ...
        with open(self.pathlist, "r") as fr:
            for line in fr.readlines():
                ...
                if self.mode != "test":
                    ...
                    ## 使用rerank dist距离
                    self.meta_weights.append([json_info["meta_label"], json_info["meta_sim"], [float(json_info["meta_dist"])]])
                    assert (target<self.num_class)

                elif self.mode == "test":
                    target = int(meta_info)
                    self.meta_weights.append([[target], [1.0], [0.0]])
    ...
    def load_target_by_txt(self):
        ...
        if self.root_dir_target != "" and \
            ...
            with open(pathlist_target, "r") as fr:
                for line in fr.readlines():
                    tf_record_offset = line.strip().split(" ")[0]
                    meta_info = line.strip().replace(tf_record_offset + " ", "")
                    tf_record = tf_record_offset.split("@")[0]
                    ...
                    meta_weights_supp.append([[target], [1.0], [0.0]])
            ...
            if self.mode == "train" and self.use_fewshot:
                ...
                if self.fast_eval:
                    ...
                    for target in target_list:
                        ...
                        for img_item in img_list_sampled:
                            ...
                            meta_weights_fewshot.append([[img_item[1]], [1.0], [0.0]])
                ...
                print("[FEW-SHOT] Load samples by pathlist from target domain")
    ...
    def get_default_sample_weight(self):
        ...
        class_nums = [0 for _ in range(self.num_class)]
        class_nums_invalid = set()
        for target in self.targets:
            if int(target) < self.num_class:
                class_nums[int(target)] += 1
            else:
                class_nums_invalid.add(int(target))
        print("unexpected class categories", class_nums_invalid)
    ...
    def get_scaled_sample_weight(self):
        ...
        class_nums = [0 for _ in range(self.num_class)]
        class_sims_sum = [0 for _ in range(self.num_class)]
        ### 重新计算每个类别的样本数目并且求反比==>top 5
        class_nums_top5 = [0 for _ in range(self.num_class)]
        class_sims_sum_top5 = [0 for _ in range(self.num_class)]
        ### 统计每个数量并得到中位数
        for target, meta_weight in zip(self.targets, self.meta_weights):
            class_nums[int(target)] += 1
            class_sims_sum[int(meta_weight[0][0])] += meta_weight[1][0]
            for meta_weight_target, meta_weight_conf in zip(meta_weight[0], meta_weight[1]):
                class_nums_top5[int(meta_weight_target)] += 1
                class_sims_sum_top5[int(meta_weight_target)] += meta_weight_conf
        ...
        ### 计算top5个类别的交叉熵损失=>使用TOP5个类别权重
        class_nums_top5, class_sims_sum_top5 = np.array(class_nums_top5), np.array(class_sims_sum_top5)
        class_median_top5 = np.median(class_nums_top5)
        class_weights_top5 = class_nums_top5 / class_median_top5
        class_scales_top5 = class_nums_top5 / class_sims_sum_top5
        class_weights_top5 *= class_scales_top5
        ### 二选一
        self.class_weights = class_weights
        # self.class_weights = class_weights_top5
        return
    ...
    def _dry_run(self):
        if self.mode == "train":
            ...
            for sample, target, domain, meta_weight in zip(self.samples,
                ...
                if target in visited_class_dry_run:
                    continue
                else:
                    visited_class_dry_run.add(target)
        ...
        print("=========><=========")
        print("[Dry-run] {} #{} samples; #{} labels; #{} domains (#{} target); #{} meta weights".format(
            self.mode, len(self.samples), len(self.targets), len(self.domains),
            np.sum(np.array(self.domains)), len(self.meta_weights)))
        print("=========><=========")
        return
        ...
        self.albs_transform_noise = [alb.ISONoise(p=0.5), alb.RandomFog(p=0.5, fog_coef_upper=0.5),\
            alb.RandomSnow(p=0.5, brightness_coeff=1.2), alb.RandomRain(p=0.5, drop_length=5),\
                alb.RandomShadow(p=0.5, num_shadows_lower=0, num_shadows_upper=1),\
                    alb.GaussNoise(p=0.5), alb.ImageCompression(quality_lower=95, p=0.5),\
                        alb.MotionBlur(p=0.5), alb.Blur(p=1),\
                            alb.GaussianBlur(p=0.5), alb.GlassBlur(sigma=0.2, p=0.5)]
        return

    def alb_transform(self, image):
        """use albumentation transform for data augmentation"""
        # if random.random() > 0.5:
        #     image = rotate_and_crop(image,\
        ...
        alb_trans = alb.Compose(alb_trans)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            if self.use_meta_weights == 2:
                ...
                target = np.random.choice(meta_label, p=meta_sim_prob)
            ...
            return img, target, img_aug, torch.Tensor([self.domains[index]]),\
            ...
            return img, target, torch.Tensor([self.domains[index]]),\
                path, torch.Tensor([self.samples_index[index]])
