...
export resume_path=${SAVE_DIR}/bopro/checkpoint_best_mAP.tar
export ANNO_PATH=${SAVE_DIR1}/pseudo_label.json

python3.9 -W ignore -u noise_cleaning.py --seed 1024 --use_meta_weights 0 --annotation ${ANNO_PATH} \
--root_dir ${root_dir} --pathlist_web ${pathlist} --pretrained --nuswide \
...
--dist-url 'tcp://localhost:10333' --multiprocessing-distributed --world-size 1 --rank 0

### add mixup by "--mixup"
python3.9 -W ignore -u train.py --seed 1024 --use_meta_weights 0 --pseudo_fewshot --annotation ${ANNO_PATH} \
--root_dir ${root_dir} --pathlist_web ${pathlist} --pretrained --nuswide --resume ${resume_path} \
--root_dir_test_web ${root_dir_test_web} --pathlist_test_web ${pathlist_test_web} \
--root_dir_test_target ${root_dir_test_target} --pathlist_test_target ${pathlist_test_target} \
--arch "resnet50" --workers 16 --epochs 20 --frozen_encoder_epoch 21 \
--init-proto-epoch 21 --start_clean_epoch 21 --batch-size ${N_BATCHSIZE} \
--topk 50 --beta 0.1 --lr 2e-5 --cos --warmup_epoch 0 \
--exp-dir ${SAVE_DIR1} --start-epoch 0 \
--num-class ${N_CLASSES} --low-dim 128 --alpha 0.5 \
--moco_queue 2048 --pseudo_th 0.9 \
--w-inst 0 --w-recn 0 --w-proto 0 \
--dist-url 'tcp://localhost:10313' --multiprocessing-distributed --world-size 1 --rank 0
